...
- Demo em Streamlit (`app.py`) com título, descrição do problema e objetivos, links úteis e tabela com amostra dos dados.
- Ambiente reprodutível: venv e `requirements.txt` na raiz da entrega.

Fica para as próximas etapas: dashboard completo (filtros, gráficos, mapas, séries históricas), WebScraping de fontes complementares, integração com LLMs para resumos em linguagem natural e análises de tendência.
...
| Utilidade analítica (etapas futuras) | Focos por estado e bioma, risco de fogo médio e FRP no dashboard | Entrega final |
...
| Data Understanding | Data Acquisition & Understanding | Data Summary Report, `fetch_inpe.py`, amostra em `data/raw/` |
| Data Preparation | Data Acquisition & Understanding | Limpeza e agregações em `data/processed/` (próximas etapas) |
...
| Deployment | Deployment | `app.py` evoluindo para o dashboard final |
...
Coleta com Python e `requests` a partir dos dados abertos do INPE; tratamento com `pandas` (`data/raw/` para `data/processed/`); apresentação em Streamlit; nas etapas futuras, WebScraping de fontes complementares e LLM via engenharia de prompts para resumos.
